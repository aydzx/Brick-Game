...
void s21::Model::preliminaryDataCheck(){
        if((game_info_.action  == UP || game_info_.action  == DOWN || game_info_.action  == RIGHT || game_info_.action  == LEFT)  && game_info_.state == SHIFTING){
        game_info_.state = MOVING;
        if(!checkPrevAction()){ // проверка на предыдущее действие (движение в противоположную сторону)
...
void s21::Model::initSnake(){
  ...
  for(std::vector<std::pair<int,int>>::size_type i = 0; i < snake.size(); i++) {
...
bool s21::Model::checkingCollision(std::pair<int, int>& newPix){
    ...
    if(newPix.first < 0 || newPix.first >= WIDTH || newPix.second < OFFSET_Y || newPix.second >= HEIGHT - 1 ) {   
...
void s21::Model::movementSnake(std::pair<int, int> &newPix){
      if(newPix.first == food.first && newPix.second == food.second) {// проверка на столкновение с яблоком
...
s21::Model::Model() {
      game_info_.state = GameState::STOP;
